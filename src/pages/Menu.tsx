...
const Menu = () => {
  ...
  const specialItems = useMemo(() => {
    return menuItems.filter(item => item.active && item.isSpecial);
  }, [menuItems]);
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      ...
      {specialItems.length > 0 && (
        <div className="mb-12">
          <h2 className="text-3xl font-serif font-bold text-center mb-6">This Week's Special</h2>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {specialItems.map(item => (
              <Card key={item.id} className="overflow-hidden border-2 border-bakery-gold">
                <div className="aspect-video relative bg-bakery-cream/20 flex items-center justify-center">
                  <h3 className="text-2xl font-serif font-bold text-bakery-brown text-center px-4">
                    {item.name}
                  </h3>
